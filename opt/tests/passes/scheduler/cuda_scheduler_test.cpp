...
TEST(CUDASchedulerTest, OuterSequentialForWithInnerMaps) {
    builder::StructuredSDFGBuilder builder("sdfg_test", FunctionType_CPU);
...
TEST(CUDASchedulerTest, ConsecutiveNestsWithInnerMapInSecondNest) {
    builder::StructuredSDFGBuilder builder("sdfg_test", FunctionType_CPU);
    ...
    auto& body_2 = loop_2.root();
    // Define inner map
    auto bound_3 = symbolic::symbol("N");
    auto indvar_3 = symbolic::symbol("i");
    auto& loop_3 = builder.add_map(
        body_2,
        indvar_3,
        symbolic::Lt(indvar_3, bound_3),
    ...
    // Add computation
    {
        auto& block = builder.add_block(loop_3.root());
        auto& a_in = builder.add_access(block, "A");
        auto& a_out = builder.add_access(block, "A");
        auto& tasklet = builder.add_tasklet(block, data_flow::TaskletCode::assign, "_out", {"_in"});
        builder.add_computational_memlet(block, a_in, tasklet, "_in", {indvar_3}, desc_2);
