...
Function::Function(const IType& return_type, bool is_var_arg,
                 DeviceLocation device_location, uint address_space,
                 const std::string& initializer)
    : return_type_(return_type.clone()), is_var_arg_(is_var_arg),
      device_location_(device_location), address_space_(address_space),
...
std::unique_ptr<IType> Function::clone() const {
    auto new_function = std::make_unique<Function>(*this->return_type_, this->is_var_arg_,
...
DeviceLocation Function::device_location() const {
    return this->device_location_;
}

uint Function::address_space() const {
    return this->address_space_;
}

std::string Function::initializer() const {
