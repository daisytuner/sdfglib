...
std::tuple<int, types::PrimitiveType> CodeGenerator::analyze_type_rec(
    symbolic::Expression* dims, int maxDim, int dimIdx, const types::IType& type, int argIdx,
    const analysis::MemAccessRange* range) {
    if (dimIdx > maxDim) {
        std::cerr << "In '" << sdfg_.name() << "', arg" << argIdx << ": data nesting deeper than "
                  << maxDim << ", ignoring" << std::endl;
    ...
    if (auto* scalarType = dynamic_cast<const types::Scalar*>(&type)) {
        ...
        if (range && !range->is_undefined()) {
            ...
            if (symbolic::eq(symbolic::zero(), dim.first)) {
                ...
                std::cerr << "In '" << sdfg_.name() << "', arg" << argIdx << " dim" << dimIdx
                          << ": has upper bound " << dim.second->__str__()
                          << ", but does not start at 0, cannot capture" << std::endl;
        ...
        std::cerr << "In '" << sdfg_.name() << "', arg" << argIdx << ": unsupported type "
                  << type.print() << ", cannot capture!" << std::endl;
        return std::make_tuple(-1, types::Void);
    }
}

bool CodeGenerator::add_capture_plan(const std::string& varName, int argIdx, bool isExternal,
    ...
    if (dimCount == 0) {
        ...
        plan.emplace_back(isRead, isWritten, CaptureVarType::Cap1D, argIdx, isExternal, innerPrim,
                          dims[0]);
    } else if (dimCount == 2) {
        plan.emplace_back(isRead, isWritten, CaptureVarType::Cap2D, argIdx, isExternal, innerPrim,
                          dims[0], dims[1]);
    } else if (dimCount == 3) {
        plan.emplace_back(isRead, isWritten, CaptureVarType::Cap3D, argIdx, isExternal, innerPrim,
