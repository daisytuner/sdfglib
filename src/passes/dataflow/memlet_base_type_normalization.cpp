...
MemletBaseTypeNormalization::MemletBaseTypeNormalization()
    : Pass(){
...
bool MemletBaseTypeNormalization::
    ...
    for (auto& state_id : sdfg.states()) {
        auto& state = sdfg.state(state_id);

        // Iterate through all data flow graphs in the state
        for (auto& dfg_id : state.data_flow_graphs()) {
            auto& dfg = state.data_flow_graph(dfg_id);

            // Collect all memlets that need normalization
            std::vector<data_flow::Memlet*> memlets_to_normalize;

            for (auto& edge : dfg.edges()) {
                auto& memlet = dfg.memlet(edge);
                ...
                if (!array_dimensions.empty()) {
                    memlets_to_normalize.push_back(&memlet);
                }
            }
            ...
            for (auto* memlet : memlets_to_normalize) {
                ...
                if (!old_subset.empty() && old_subset.size() <= array_dimensions.size()) {
                    // Calculate linearized index: idx = i0 * (d1 * d2 * ... * dn-1) + i1 * (d2 * ... * dn-1) + ... +
                    // in-1
