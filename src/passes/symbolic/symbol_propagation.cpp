    ...
    for (auto& name : sdfg.containers()) {
        ...
        for (auto& entry : raw_groups) {
            ...
            for (auto& user : middle_users) {
                ...
                auto inverse = symbolic::inverse(sym_lhs, sym_rhs);
                if (inverse == SymEngine::null) {
                    success = false;
                    break;
                }

                rhs_modified = symbolic::subs(rhs_modified, sym_lhs, inverse);
            ...
            if (auto transition_stmt =
                ...
                for (size_t i = 0; i < if_else_stmt->size(); i++) {
                    ...
                    if (symbolic::uses(child.second, lhs)) {
                        ...
                        child.second = symbolic::simplify(child.second);
                        applied = true;
                    }
                }
                ...
                if (symbolic::uses(condition, lhs)) {
                    ...
                    tasklet->condition() = symbolic::simplify(tasklet->condition());
                    applied = true;
                }
                ...
                if (for_user->is_init() && symbolic::uses(for_loop->init(), lhs)) {
                    ...
                    for_loop->condition() = symbolic::simplify(for_loop->condition());
                    applied = true;
