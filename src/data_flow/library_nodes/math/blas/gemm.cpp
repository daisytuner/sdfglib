...
bool GEMMNode::expand(builder::StructuredSDFGBuilder& builder, analysis::AnalysisManager& analysis_manager) {
    ...
    builder.add_computational_memlet(init_block, zero_node, init_tasklet, "_in", {}, debug_info);
    builder.add_computational_memlet(init_block, init_tasklet, "_out", sum_init, {}, debug_info);
    ...
    auto& core_fma = builder.add_tasklet(code_block, data_flow::fma, "_out", {"_in1", "_in2", "_in3"}, debug_info);
    auto& sum_in = builder.add_access(code_block, sum_var, debug_info);
    auto& sum_out = builder.add_access(code_block, sum_var, debug_info);
    builder.add_computational_memlet(code_block, sum_in, core_fma, "_in3", {}, debug_info);
    ...
    auto& scale_sum_tasklet = builder.add_tasklet(flush_block, data_flow::mul, "_out", {"_in1", "_in2"}, debug_info);
    builder.add_computational_memlet(flush_block, sum_final, scale_sum_tasklet, "_in1", {}, debug_info);
    if (auto const_node = dynamic_cast<data_flow::ConstantNode*>(alpha_node)) {
        auto& alpha_node_new = builder.add_constant(flush_block, const_node->data(), const_node->type(), debug_info);
        builder.add_computational_memlet(flush_block, alpha_node_new, scale_sum_tasklet, "_in2", {}, debug_info);
    } else {
        auto& alpha_node_new = builder.add_access(flush_block, alpha_node->data(), debug_info);
        builder.add_computational_memlet(flush_block, alpha_node_new, scale_sum_tasklet, "_in2", {}, debug_info);
    }

    std::string scaled_sum_temp = builder.find_new_name("scaled_sum_temp");
    builder.add_container(scaled_sum_temp, scalar_type);
    auto& scaled_sum_final = builder.add_access(flush_block, scaled_sum_temp, debug_info);
    builder
        .add_computational_memlet(flush_block, scale_sum_tasklet, "_out", scaled_sum_final, {}, scalar_type, debug_info);

    auto& scale_input_tasklet = builder.add_tasklet(flush_block, data_flow::mul, "_out", {"_in1", "_in2"}, debug_info);
    builder.add_computational_memlet(
        flush_block,
        input_node_c_new,
        scale_input_tasklet,
    ...
    if (auto const_node = dynamic_cast<data_flow::ConstantNode*>(beta_node)) {
        auto& beta_node_new = builder.add_constant(flush_block, const_node->data(), const_node->type(), debug_info);
        builder.add_computational_memlet(flush_block, beta_node_new, scale_input_tasklet, "_in2", {}, debug_info);
    } else {
        auto& beta_node_new = builder.add_access(flush_block, beta_node->data(), debug_info);
        builder.add_computational_memlet(flush_block, beta_node_new, scale_input_tasklet, "_in2", {}, debug_info);
    }

    std::string scaled_input_temp = builder.find_new_name("scaled_input_temp");
    builder.add_container(scaled_input_temp, scalar_type);
    auto& scaled_input_c = builder.add_access(flush_block, scaled_input_temp, debug_info);
    builder
        .add_computational_memlet(flush_block, scale_input_tasklet, "_out", scaled_input_c, {}, scalar_type, debug_info);

    auto& flush_add_tasklet = builder.add_tasklet(flush_block, data_flow::add, "_out", {"_in1", "_in2"}, debug_info);
    ...
    builder
        .add_computational_memlet(flush_block, scaled_sum_final, flush_add_tasklet, "_in1", {}, scalar_type, debug_info);
    builder
        .add_computational_memlet(flush_block, scaled_input_c, flush_add_tasklet, "_in2", {}, scalar_type, debug_info);
    builder.add_computational_memlet(
        flush_block,
        flush_add_tasklet,
        "_out",
