...
LayerNormalizationNode::LayerNormalizationNode(
    ...
    : MathNode(
          element_id, debug_info, vertex, parent, LibraryNodeType_LayerNormalization, {"Y"}, {"X", "Scale", "B"}, data_flow::ImplementationType_NONE
      ),
...
bool LayerNormalizationNode::expand(builder::StructuredSDFGBuilder &builder, analysis::AnalysisManager &analysis_manager) {
    ...
    builder.add_computational_memlet(compute_block, input_access, add_mean_tasklet, "_in1", input_subset, iedge_input->base_type());
    ...
    builder.add_computational_memlet(compute_block, input_access, square_tasklet, "_in1", input_subset, iedge_input->base_type());
    builder.add_computational_memlet(compute_block, input_access, square_tasklet, "_in2", input_subset, iedge_input->base_type());
    ...
    if (iedge_bias) {
        ...
        builder.add_computational_memlet(norm_block, assign_tasklet, "_out", output_access_norm, loop_syms, oedge_output->base_type());
    }
...
std::unique_ptr<data_flow::DataFlowNode> LayerNormalizationNode::
    ...
    return std::unique_ptr<data_flow::DataFlowNode>(new LayerNormalizationNode(
