...
TEST(StructuredSDFGDeepCopy, Block) {
    builder::StructuredSDFGBuilder builder_source("sdfg_source", FunctionType_CPU, DebugTable());
    auto& sdfg_source = builder_source.subject();
    auto& root_source = sdfg_source.root();

    auto& block = builder_source.add_block(root_source);

    builder::StructuredSDFGBuilder builder_target("sdfg_target", FunctionType_CPU, DebugTable());
