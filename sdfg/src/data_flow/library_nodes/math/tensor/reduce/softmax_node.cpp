...
bool SoftmaxNode::expand(builder::StructuredSDFGBuilder& builder, analysis::AnalysisManager& analysis_manager) {
    ...
    // 3. Exp(TmpSub) -> TmpExp
    {
        ...
        auto& exp_node = builder.add_library_node<math::tensor::ElementwiseNode>(
            exp_block, exp_block.debug_info(), math::cmath::CMathFunction::exp, element_type.primitive_type()
        );
    ...
    // 5. Div(TmpExp, TmpSumBcast) -> Output
    {
        ...
        auto& div_node = builder.add_library_node<
            math::tensor::ElementwiseNode>(div_block, div_block.debug_info(), data_flow::TaskletCode::fp_div);
