...
bool DataTransferMinimization::accept(structured_control_flow::Sequence& sequence) {
    bool applied = false;
    memory::OffloadingNode* copy_out = nullptr;
    ...
    while (copy_out_index < sequence.size()) {
        ...
        for (i = copy_out_index; i < sequence.size(); i++) {
            ...
            auto* copy_in = dynamic_cast<memory::OffloadingNode*>(*copy_in_block->dataflow().library_nodes().begin());
            if (!copy_in) {
                continue;
            }
...
std::pair<data_flow::AccessNode*, data_flow::AccessNode*> DataTransferMinimization::
    get_src_and_dst(data_flow::DataFlowGraph& dfg, memory::OffloadingNode* offloading_node) {
    ...
    if (dynamic_cast<cuda::CUDAOffloadingNode*>(offloading_node) ||
        dynamic_cast<tenstorrent::TTOffloadingNode*>(offloading_node)) {
        src = this->get_in_access(offloading_node, "_src");
        dst = this->get_out_access(offloading_node, "_dst");
    } else if (auto* external_offload = dynamic_cast<memory::ExternalOffloadingNode*>(offloading_node)) {
