...
class OutermostLoopsInstrumentation : public Instrumentation {
   ...
   public:
    OutermostLoopsInstrumentation(StructuredSDFG& sdfg);

    bool should_instrument(const structured_control_flow::ControlFlowNode& node) const override;

    void begin_instrumentation(const structured_control_flow::ControlFlowNode& node,
                               PrettyPrinter& stream) const override;

    void end_instrumentation(const structured_control_flow::ControlFlowNode& node,
                             PrettyPrinter& stream) const override;
};
